    ...
    def run(
        ...
        if os.name != "nt" or not self.executable:
            return WindowsExtendedResult(
                "powershell",
                WindowsExtendedState.UNAVAILABLE,
                normalized_error="POWERSHELL_UNAVAILABLE",
            )
...
class ExtendedWindowsRuntime:
    ...
    def network_adapters(self) -> WindowsExtendedResult:
        ...
        return WindowsExtendedResult(
            "network_adapters", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def wifi_profiles(self) -> WindowsExtendedResult:
        ...
        result = self.runner.run(script)
        return WindowsExtendedResult(
            "wifi_profiles", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def bluetooth_devices(self) -> WindowsExtendedResult:
        ...
        result = self.runner.run(script)
        return WindowsExtendedResult(
            "bluetooth_devices", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def printers(self) -> WindowsExtendedResult:
        ...
        result = self.runner.run(script)
        return WindowsExtendedResult(
            "printers", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def services(self, name_filter: str = "") -> WindowsExtendedResult:
        ...
        return WindowsExtendedResult(
            "services", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def clipboard_read(self) -> WindowsExtendedResult:
        ...
        result = self.runner.run(script)
        return WindowsExtendedResult(
            "clipboard_read", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def clipboard_write(self, text: str) -> WindowsExtendedResult:
        if len(text) > 100_000:
            return WindowsExtendedResult(
                "clipboard_write",
                WindowsExtendedState.DENIED,
                normalized_error="CLIPBOARD_TEXT_TOO_LARGE",
            )
        ...
        return WindowsExtendedResult(
            "clipboard_write", state, result.data, result.message, result.normalized_error
        )
    ...
    def set_service_state(self, service_name: str, running: bool) -> WindowsExtendedResult:
        if not service_name.strip() or len(service_name) > 128:
            return WindowsExtendedResult(
                "service_state",
                WindowsExtendedState.DENIED,
                normalized_error="INVALID_SERVICE_NAME",
            )
        ...
        return WindowsExtendedResult(
            "service_state", result.state, result.data, result.message, result.normalized_error
        )

    def set_network_adapter_enabled(
        self, adapter_name: str, enabled: bool
    ) -> WindowsExtendedResult:
        if not adapter_name.strip() or len(adapter_name) > 128:
            return WindowsExtendedResult(
                "network_adapter",
                WindowsExtendedState.DENIED,
                normalized_error="INVALID_ADAPTER_NAME",
            )
        ...
        return WindowsExtendedResult(
            "network_adapter", result.state, result.data, result.message, result.normalized_error
        )
    ...
    def open_settings(self, page: str) -> WindowsExtendedResult:
        ...
        if target is None:
            return WindowsExtendedResult(
                "open_settings",
                WindowsExtendedState.DENIED,
                normalized_error="SETTINGS_PAGE_NOT_ALLOWED",
            )
        if os.name != "nt":
            return WindowsExtendedResult(
                "open_settings",
                WindowsExtendedState.UNAVAILABLE,
                normalized_error="WINDOWS_REQUIRED",
            )
        try:
            ...
            return WindowsExtendedResult(
                ...
                WindowsExtendedState.FAILED,
                message=str(error),
                normalized_error="SETTINGS_OPEN_FAILED",
            )
        return WindowsExtendedResult(
            "open_settings",
            WindowsExtendedState.UNVERIFIED,
            data={"uri": target},
            message="Settings page launch requested",
        )
